  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes("index")) {
        setError(
          "Error de configuración de índices en Firebase. Contacta al admin."
        );
      } else {
    ...
    <div className="page-content">
      ...
      >
        ...
        <select
          ...
          style={{
            ...
            borderRadius: "8px",
            flex: 1,
            opacity: searchTerm.length >= 3 ? 0.5 : 1,
          }}
        >
          {Object.entries(ORDER_OPTIONS).map(([k, v]) => (
      ...
      {loading && (
        <p style={{ textAlign: "center" }}>
          <FaSpinner className="spin-icon" /> Buscando...
        </p>
      )}
      {error && (
        <p style={{ textAlign: "center", color: "var(--error-color)" }}>
          {error}
        </p>
      )}
