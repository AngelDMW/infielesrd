...
import { 
    ...
    updateDoc, increment, deleteDoc, setDoc, serverTimestamp, addDoc, getCountFromServer,
    runTransaction // 🏆 CORRECCIÓN CRÍTICA 2: Añadir 'runTransaction' (necesario para la lógica de likes)
} from "firebase/firestore";
import { db } from "../firebase";
import { FaHeart, FaCommentAlt, FaRegClock, FaUserSecret, FaReply, FaArrowLeft, FaFlag, FaChevronDown, FaChevronUp, FaSpinner } from 'react-icons/fa';
...
const ReplyForm = ({ parentCommentId, replyToReplyId, storyId, handleCommentUpdate, onReplySuccess }) => {
    ...
    const handleReplySubmit = async (e) => {
        ...
        try {
            const repliesRef = collection(db, "stories", storyId, "comments", parentCommentId, "replies");
            ...
            await updateDoc(doc(db, "stories", storyId, "comments", parentCommentId), {
    ...
    return (
        <form onSubmit={handleReplySubmit} style={{ margin: '10px 0', borderLeft: '3px solid var(--border)', paddingLeft: '10px' }}>
            <textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder={replyToReplyId ? "Respuesta a otro anónimo..." : "Escribe una respuesta..."}
                rows="2"
                style={{
                    width: '100%',
                    padding: '8px',
            ...
            />
            <button type="submit" className="btn-primary" style={{ padding: '5px 10px', fontSize: '0.8rem', width: 'auto' }}>
                Responder
            </button>
        </form>
    );
...
const CommentCard = ({ comment, storyId, handleReport, handleCommentUpdate }) => {
    ...
    const loadReplies = useCallback(async () => {
        ...
        try {
            const repliesRef = collection(db, "stories", storyId, "comments", comment.id, "replies");
    ...
    return (
        <div className="comment-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                <span style={{ fontSize: '0.9rem', fontWeight: 'bold', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '5px' }}>
            ...
            <p style={{ margin: '0 0 10px 0', whiteSpace: 'pre-wrap' }}>{comment.content}</p>
            
            <div style={{ display: 'flex', gap: '15px', fontSize: '0.85rem', color: 'var(--nav-link)', alignItems: 'center' }}>
                <span onClick={() => handleReplyTo()} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <FaReply /> Responder
                </span>
                <span onClick={() => handleReport('comment', comment.id, comment.content.substring(0, 50))} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <FaFlag /> Reportar
                </span>
                ...
                {(comment.repliesCount > 0 || replies.length > 0) && (
                    <span 
                        ...
                        style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', marginLeft: 'auto' }}
                    >
                        {showReplies ? <FaChevronUp /> : <FaChevronDown />} 
                        {comment.repliesCount || replies.length} {comment.repliesCount === 1 ? 'Respuesta' : 'Respuestas'}
                    </span>
                )}
            </div>
            ...
            {showReplies && (
                <div style={{ marginTop: '15px', borderLeft: '2px solid var(--card-border)', paddingLeft: '10px' }}>
                    {isLoadingReplies && <p style={{ fontSize: '0.8rem', color: 'var(--nav-link)' }}>Cargando respuestas...</p>}
                    {!isLoadingReplies && replies.length === 0 && (
                         <p style={{ fontSize: '0.8rem', color: 'var(--nav-link)' }}>No hay respuestas.</p>
                    )}
                    
                    {replies.map(reply => (
                        <div key={reply.id} style={{ marginBottom: '10px', padding: '5px', background: 'var(--secondary)', borderRadius: '8px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '3px' }}>
                                <span style={{ fontWeight: 'bold', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                    <FaUserSecret /> Anónimo
                                </span>
                            ...
                            <p style={{ margin: '0 0 5px 0', fontSize: '0.9rem', whiteSpace: 'pre-wrap' }}>{reply.content}</p>
                            <div style={{ display: 'flex', gap: '10px', fontSize: '0.8rem', color: 'var(--nav-link)' }}>
                                <span onClick={() => handleReplyTo(reply.id)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                    <FaReply /> Responder
                                </span>
                                <span onClick={() => handleReport('reply', reply.id, reply.content.substring(0, 50))} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                    <FaFlag /> Reportar
                                </span>
                            </div>
                        </div>
...
export default function StoryView() {
    ...
    const [reportModal, setReportModal] = useState({ visible: false, type: '', id: '', snippet: '' });
    ...
    const fetchStory = useCallback(async () => {
        ...
        try {
            ...
            if (storyData.status !== 'approved') {
                setError("La historia no ha sido publicada o fue eliminada por el administrador.");
                setLoading(false);
            ...
            setError("Ocurrió un error al intentar cargar la historia. Intenta más tarde.");
        } finally {
            setLoading(false);
        }
    ...
    const handleCommentSubmit = async (e) => {
        ...
        try {
            ...
            setComments(prev => [
                { id: docRef.id, ...newCommentData, createdAt: { toDate: () => new Date() } }, // Usar una estructura de Date para el formato en el front
                ...prev
            ]);
    ...
    if (loading) {
        return (
            <div className="page-content" style={{ textAlign: 'center', paddingTop: '50px' }}>
                <FaSpinner className="spinner" size={30} style={{ color: 'var(--primary)' }} />
                <p>Cargando chisme...</p>
            </div>
        );
    }
    ...
    const ReportModal = () => (
        <div style={{
            ...
            <form onSubmit={submitReport} className="card" style={{ padding: '20px', width: '90%', maxWidth: '400px', margin: '20px' }}>
                <h2>Reportar Contenido 🚨</h2>
                <p style={{fontSize: '0.9rem', color: 'var(--text-secondary)'}}>
                    Estás reportando {reportModal.type === 'story' ? 'la historia' : `el ${reportModal.type}`} con fragmento: 
                    <em style={{display: 'block', margin: '5px 0', fontWeight: 'bold'}}>&quot;{reportModal.snippet}...&quot;</em>
                </p>
    ...
    return (
        <div className="page-content">
            ...
            {/* Header de Historia */}
            <div className="story-header" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px'}}>
                <Link to="/stories" style={{textDecoration: 'none', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '5px', fontWeight: 'bold'}}>
                    <FaArrowLeft /> Archivo
                </Link>
                <span style={{background: 'var(--primary)', color: 'white', padding: '5px 10px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold'}}>
                    {categoryLabel}
                </span>
            </div>
            ...
            <div className="story-meta" style={{ display: 'flex', gap: '15px', fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '20px' }}>
                <span style={{display: 'flex', alignItems: 'center', gap: '5px'}}><FaRegClock /> {formatTimeAgo(story.publishedAt)}</span>
                <span style={{display: 'flex', alignItems: 'center', gap: '5px'}}><FaUserSecret /> Anónimo</span>
            </div>
            ...
            <div className="story-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 0', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', margin: '20px 0' }}>
                
                <button 
                    onClick={handleLike} 
                    style={{ background: 'none', border: 'none', color: isLiked ? 'var(--primary)' : 'var(--nav-link)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold' }}
                >
                    <FaHeart size={20} />
                ...
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    <span style={{display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--nav-link)', fontSize: '1rem', fontWeight: 'bold'}}>
                        <FaCommentAlt size={20} />
                        {story.commentsCount || 0} Comentarios
                    </span>
                    <button 
                        onClick={() => handleReport('story', story.id, story.title)} 
                        style={{ background: 'none', border: '1px solid var(--error-color)', color: 'var(--error-color)', padding: '5px 10px', borderRadius: '15px', display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', fontSize: '0.9rem' }}
                    >
                        <FaFlag /> Reportar
                    </button>
                </div>
            ...
            <h2 style={{ fontSize: '1.5rem', marginTop: '30px', marginBottom: '20px', borderBottom: '2px solid var(--primary)', paddingBottom: '5px' }}>
                <FaCommentAlt style={{marginRight: '10px', color: 'var(--primary)'}}/> Comentarios
            </h2>
            ...
            <div className="comments-list">
                {comments.length > 0 ? (
                    ...
                    <p style={{textAlign: 'center', color: 'var(--text-secondary)'}}>Sé el primero en comentar este chisme.</p>
                )}
            </div>
