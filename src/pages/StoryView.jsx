...
export default function StoryView() {
  ...
  const handleLike = async () => {
    ...
    setStory((prev) => ({
      ...prev,
      likes: prevLiked ? prev.likes - 1 : prev.likes + 1,
    }));
    ...
    try {
      await runTransaction(db, async (t) => {
        ...
        if (sfDoc.exists())
          t.update(storyRef, {
            likes: prevLiked ? increment(-1) : increment(1),
          });
      });
    } catch (e) {
      console.error(e);
  ...
  const handleSendComment = async (e) => {
    ...
    try {
      ...
      setComments((prev) => [
        ...prev,
        { id: docRef.id, ...newComment, createdAt: new Date() },
      ]);
      setStory((prev) => ({
        ...prev,
      ...
      <main style={{ marginTop: "20px", padding: "0 20px" }}>
        ...
        >
          <button
            ...
            style={{
              flex: 2,
              background: isLiked ? "rgba(217,4,41,0.08)" : "var(--bg-body)",
              border: "none",
              borderRadius: "12px",
              padding: "12px",
              display: "flex",
          ...
          >
            <div
              style={{
                transform: animateLike ? "scale(1.4)" : "scale(1)",
                transition: "0.2s",
              }}
            >
